      ...
      <div class="row">
        ...
        <div class="col-md-12">
          <div class="card card-primary card-outline">
            <div class="card-header">
              <h3 class="card-title">Blotter inbox</h3>
            ...
            <div class="card-body p-0">
              ...
              <div class="table-responsive mailbox-messages">
                <table class="table table-hover table-striped">
                  <tbody>
                  <?php 
                    $get_c = mysqli_query($conn, "SELECT * FROM blotter b JOIN users u ON b.added_by=u.user_Id ORDER BY is_read ASC, date_added DESC");
                    if(mysqli_num_rows($get_c) > 0) {
                      while ($row2 = mysqli_fetch_array($get_c)) {
                  ?>
                    <tr <?php echo ($row2['is_read'] == 0) ? 'style="font-weight: bold;"' : 'class="text-muted"'; ?> data-searchable="<?= strtolower($row2['firstname'].' '.$row2['lastname'].' '.$row2['incidentNature'].' '.$row2['incidentDescription']); ?>">

                      <td>
                          <div class="icheck-primary">
                              <input type="checkbox" value="<?= $row2['blotter_Id']; ?>" id="check<?= $row2['blotter_Id']; ?>" <?php if($row2['is_read'] == 1) { echo 'disabled'; }  ?>>
                              <label for="check<?= $row2['blotter_Id']; ?>"></label>
                          </div>
                      </td>
                      <td class="mailbox-name"><a href="blotter_view.php?blotter_Id=<?= $row2['blotter_Id']; ?>"><?= ucwords($row2['firstname'].' '.$row2['lastname']) ?></a></td>
                      <td class="mailbox-subject"><b><?= ucwords($row2['incidentNature']) ?></b> - <?php echo ucwords(substr($row2['incidentDescription'], 0, 30)) . '...'; ?>
                      </td>
  ...
  $(document).ready(function() {
    $('.checkbox-toggle').on('click', function() {
      ...
      if (complaintIds.length > 0) {
        ...
        alert('Please select at least one blotter record to mark as read.');
      }
    });
