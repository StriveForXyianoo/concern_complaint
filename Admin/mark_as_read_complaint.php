<?php 
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['complaint_ids'])) {
		foreach($_POST['complaint_ids'] as $complaintId){
			$sql = "SELECT * FROM complaint WHERE complaint_ID ='$complaintId'";
			$result = mysqli_query($conn,$sql);
			$row = mysqli_fetch_assoc($result);
			$complaintNature = $row['complaint_nature'];
			$incidentDate = $row['date_happened'];
			$incidentTime = $row['time_happened'];
			$incidentAddress = $row['incident_location'];
			$added_by = $row['added_by'];
			$bysql = "SELECT * FROM users  WHERE user_Id='$added_by'";
			$byresult = mysqli_query($conn,$bysql);
			$byrow = mysqli_fetch_assoc($byresult);
			$contact = $byrow['contact'];
			$name = $byrow['firstname'].' '.$byrow['lastname'];
			$updatesql = "UPDATE complaint SET is_read = 1 WHERE complaint_ID='$complaintId'";
			$updateResult = mysqli_query($conn,$updatesql);
			  $ch = curl_init();
			  $message = "Good day " . ucwords($name) . "!\nYour complaint has been read by the admin.\n\nDetails:\n";
			  $message .= "Complaint Nature: " . $complaintNature . "\n";
	...
	//   $getUserInfoQuery = "SELECT u.firstname, u.lastname, u.contact, c.added_by, c.complaint_nature, c.incident_location, c.date_happened, c.time_happened
	//                         FROM users u
	//                         JOIN complaint c ON u.user_Id = c.added_by
	//                         WHERE c.complaint_ID IN (" . implode(',', $complaintIds) . ")";
	...
	//       $complaintNature = $userInfo['complaint_nature'];
	//       $incidentLocation = $userInfo['incident_location'];
	//       $dateHappened = $userInfo['date_happened'];
	//       $timeHappened = $userInfo['time_happened'];
	...
	//       if ($result) {
	//       	echo 'success';
	//         sendSMS($name, $contact, $complaintNature, $incidentLocation, $dateHappened, $timeHappened);
